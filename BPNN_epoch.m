...
dw_o2h_epoch=zeros(output_cell,hidden_cell);

dw_h2i=zeros(hidden_cell,input_cell );
dw_h2i_epoch=zeros(hidden_cell,input_cell);

%=========================Training=============================
for epoch=1:iter    
    for num=1:batch % #1~#batch data
        %=======================Forward===========================
        ...
        dw_o2h_epoch = dw_o2h_epoch + dw_o2h;
        dw_h2i_epoch = dw_h2i_epoch + dw_h2i;        
        
    end
    dw_o2h_epoch = dw_o2h_epoch / batch;
    dw_h2i_epoch = dw_h2i_epoch / batch;
    
    w_o2h = w_o2h + dw_o2h_epoch;
    w_h2i = w_h2i + dw_h2i_epoch;    
...
%%
figure(1)
subplot(211),plot(1:epoch,E_ave),
title(['Training Loss:',num2str(E_ave(epoch)) ,'  Hidden cell :',num2str(hidden_cell),'  \eta:',num2str(learning_rate),'  \alpha:',num2str(alpha)]);
xlabel('epoch'),ylabel('E ave')
subplot(212),stem(301:400,E(301:400),'.'),
title(['test Loss :',num2str(mean(E(301:400)))]);
xlabel('number of data'),ylabel('E')
suptitle(['E test - E train = ',num2str(mean(E(301:400))-E_ave(epoch))]);
